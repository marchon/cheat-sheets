...
Cloned from https://en.wikipedia.org/wiki/SVG.
...
== Thumbnailers ==
While SVG has received widespread adoption in the market as the de facto standard for scalable vector graphics, the files are still not natively supported in Windows, so users must download an SVG preview thumbnailer for the File Explorer or use an alternate file explorer with native support.
