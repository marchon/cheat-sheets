...
Cloned from https://en.wikipedia.org/wiki/Scalability.
...
Doubling the processing power has only sped up the process by roughly one-fifth. If the whole problem were parallelizable, the speed would also double. Therefore, throwing in more hardware is not necessarily the optimal approach.
