...
Cloned from https://en.wikipedia.org/wiki/Continuous_integration.
...
=== Atomic commits ===
CI requires the version control system to support atomic commits; in other words, all of a developer's changes are handled as a single commit.
...
use test-driven development and
...
Proponents of CI recommend storing all files and information needed for building in version control (for git a repository), and that the system should be buildable from a fresh checkout and not require additional dependencies. 
...
Continuous design – Design process
Continuous testing – Automated testing process in software development
