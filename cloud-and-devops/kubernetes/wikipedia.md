...
Cloned from https://en.wikipedia.org/wiki/Kubernetes.
...
=== Control plane ===
The Kubernetes control plane manages its workload and directs communication across the system. It consists of various components such as TLS encryption, role-based access control (RBAC) and a strong authentication method, network separation, each its own process, that can run both on a single node or on multiple nodes supporting high-availability clusters. The various components of the Kubernetes control plane are as follows.
...
Originally, kubelet was tightly coupled to Docker as a container runtime.  Later it developed CRI and a "dockershim" that interfaced with Docker on one side, and (via CRI) the Kubelet on the other side. From November 2020 up to April 2022, Kubernetes has deprecated the shim. With the release of v1.24 in May 2022, the "dockershim" has been removed entirely, dropping official support for Docker as a container runtime.
As of 2026, widely used CRI-compliant container runtimes include containerd and CRI-O.
...
=== Pods ===
The basic deployable unit of computing in Kubernetes is the pod, which consists of one or more containers co-located on the same node. Each pod is assigned a unique IP address within the cluster, allowing applications to use ports without the risk of conflict. Within the pod, all containers have a shared context and can reference each other.
A container resides inside a pod. The container is the lowest level of a micro-service, which holds the running application, libraries, and their dependencies.  In the most common use case, each pod has a single container.
...
=== Volumes ===
Filesystems in a Kubernetes container provide ephemeral storage, by default. This means that a restart of the container will wipe out its filesystem. Therefore, this form of storage is quite limiting in anything but trivial applications. A Kubernetes volume provides persistent storage. This storage can also be used as shared disk space for containers within the pod. Volumes are mounted in the container at specific mount points defined by the pod configuration, and cannot mount onto other volumes or link to other volumes. The same volume can be mounted at different points in the file system tree by different containers.
...
uid: a string that uniquely identifies any API object in the cluster, regardless of type, name, namespace and time -- that is, even across deletions and recreations with the same type, name and namespace.
...
SUSE Rancher Prime, including k3s and RKE2 Kubernetes distributions
