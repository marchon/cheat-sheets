...
Cloned from https://en.wikipedia.org/wiki/Ruby_(programming_language).
...
Full support for grapheme segmentation (UAX #29). Unlike the competitor Python which requires installing a third party library. puts("🏴󠁧󠁢󠁥󠁮󠁧󠁿".grapheme_clusters.length) # 1
...
As of 2026, there are a number of alternative implementations of Ruby, including JRuby, Rubinius, and mruby. Each takes a different approach, with JRuby and Rubinius providing just-in-time compilation and mruby also providing ahead-of-time compilation.
Ruby has three major alternative implementations:

JRuby, a mixed Java and Ruby implementation that runs on the Java virtual machine. JRuby currently targets Ruby 3.4.
...
mruby, an implementation designed to be embedded into C code, in a similar vein to Lua. It is being developed by Yukihiro Matsumoto and others
...
=== Strings and Symbols ===
Ruby has multiple string types which can cause problems. Both are garbage collected since Ruby 2.2. Before Ruby 2.2 creating lots of symbols would just leak memory.

Symbols are identical to Lua-strings which are immutable, hashed and only exist once in memory with many references to it and thus quick to compare.
Strings on the other hand are a separate object each time they occur in the program with their own mutable data storage that is not hashed and thus quick to create and manipulate.
Strings come in many encodings. Strings of different encodings are also incompatible and throw an error when joined. in `join': incompatible character encodings: UTF-8 and ASCII-8BIT (Encoding::CompatibilityError). This can happen easily because many functions return strings as ASCII-8BIT by default. The programmer has to manually set the encoding of the string with .force_encoding(Encoding::UTF_8) when such strings are encountered. The character encodings UTF-8 and ASCII-8BIT are not actually incompatible and usually both contain UTF-8. Throwing an error creates a lot more bugs than it prevents.
However having multiple string types creates problems. Among others that associative arrays (Hash) in Ruby have separate indices for "strings" and :symbols of the same value and "a" is not equal to :a requiring conversion between types.


=== Significant whitespace ===
Just like Python, Ruby is a language with significant whitespace, which can cause subtle bugs if users aren't aware of where it belongs and where it doesn't.

This can even cause different results without error:


=== Multiple comparison operators ===
Like C and C++ (C alternative tokens) Ruby has alternative comparison operators: and alongside && and or alongside ||. However their behavior is vastly different which confuses all newcomers. and and or have lower precedence than assignments and function calls.
