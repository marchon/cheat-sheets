...
Cloned from https://en.wikipedia.org/wiki/HTTP.
...
QUERY
Requests that the server process the enclosed content in a safe and idempotent manner and then respond with the result. This is similar to POST requests, but QUERY requests can be automatically repeated or restarted without concern for partial state changes.
