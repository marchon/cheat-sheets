...
Cloned from https://en.wikipedia.org/wiki/Apache_Hadoop.
...
The Hadoop distributed file system (HDFS) is a distributed, scalable, and portable file system written in Java for the Hadoop framework. A Hadoop instance is divided into HDFS and MapReduce. HDFS is used for storing the data and MapReduce is used for processing data.
...
YARN strives to efficiently allocate resources to several different applications at once. It runs two daemons, which take care of two different tasks: the resource manager, responsible for job tracking and application resource-allocation; and the application master, which monitors execution progress.
...
== Papers ==
Influential papers on the birth, growth, and curation of Hadoop and big data processing include:
