...
Cloned from https://en.wikipedia.org/wiki/IBM_Db2.
...
== Editions ==
IBM offers four editions of Db2: Community Edition, Starter Edition, Standard Edition, and Advanced Server Edition.


=== IBM Db2 Community Edition ===
IBM Db2 Community Edition is a free-to-download, free-to-use edition of the IBM Db2 database. Version 11.5 provides all core capabilities of Db2 but is limited to 4 virtual processor cores, 16 GB of instance memory, has no enterprise-level support, and no fix packs. Version 12.1, on the other hand, further limits the server memory to only 8 GB per virtual or physical server, and  restricts use to non-production environments.
While there is no limit on the database size since version 11.5.1, some previous releases imposed a limit of 100 GB. A prior free Db2 version, the IBM DB2 Express-C, supported up to 16 GB RAM and two CPU cores.


=== Db2 Starter Edition ===
The Starter edition was introduced with Db2 version 12.1. It is a paid product, and has several limitations, including a limitation to 4 cores and 16 GB of memory, and restrictions on advanced functionality such as native encryption, audit logging, federation, Db2 pureScale, etc.
...
Db2 can be administered from either the command-line or a GUI. Db2 supports both SQL and XQuery. DB2 has a native implementation of XML data storage, where XML data is stored as XML (not as relational data or CLOB data) for faster access using XQuery.
